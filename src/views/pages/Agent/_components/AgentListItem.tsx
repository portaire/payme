...
function AgentListItem({ agent }:AgentListItemProps) {
    ...
    return (
    <article>
    <Link to={`/agents/${agent._id}`} className={`transition duration-200 ease-in-out bg-[#f2efe9]/80 md:flex md:items-center md:justify-between md:space-x-5 p-5 rounded-md ${isActive  ? "cursor-pointer hover:bg-[#f2efe9]" : "opacity-50 cursor-not-allowed"}`}>
